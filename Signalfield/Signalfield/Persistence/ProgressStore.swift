// Signalfield/Persistence/ProgressStore.swift
//
// Persistence layer for campaign progress.
// Saves and loads a small JSON file from ~/Library/Application Support/Signalfield/.
...
struct LevelRecord: Codable {
    ...
    var bestStars: Int = 0
    /// Best composite score achieved across all runs.
    var bestScore: Int = 0
    /// Fastest completion time in seconds. nil = never completed.
    var bestTimeSeconds: Double? = nil
}
...
///
/// Owned once as `@StateObject` in `SignalfieldApp` and shared as
...
final class ProgressStore: ObservableObject {
    ...
    ///
    /// Updates `completed`, `bestStars`, `bestScore`, and `bestTimeSeconds`
    /// using best-of logic — existing bests are never overwritten by worse values.
    /// Writes to disk immediately after any improvement.
    func recordResult(levelId: String, score: Int, timeSeconds: Double, stars: Int) {
        ...
        print("[ProgressStore] 📝 recordResult(\(levelId)) — ⭐\(stars) score:\(score) time:\(timeSeconds)  total records: \(data.levelRecords.count + 1)\n  \(caller)")
        ...
        if let existing = record.bestTimeSeconds {
            if timeSeconds < existing { record.bestTimeSeconds = timeSeconds }
        } else {
    ...
    /// Returns the player's best score for a level (0 if never completed).
    func bestScore(for levelId: String) -> Int {
        data.levelRecords[levelId]?.bestScore ?? 0
    }

    /// Returns the player's best completion time for a level (nil if never completed).
    func bestTime(for levelId: String) -> Double? {
        data.levelRecords[levelId]?.bestTimeSeconds
    }
    ...
    func dumpRecords() {
        ...
        for (id, rec) in sorted {
            let t = rec.bestTimeSeconds.map { String(format: "%.1f", $0) } ?? "nil"
            print("  \(id): ⭐\(rec.bestStars)  score:\(rec.bestScore)  time:\(t)  completed:\(rec.completed)")
        }
