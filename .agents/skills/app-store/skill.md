...
## Workflow
1. Identify the request type: submission readiness, metadata drafting, pricing, TestFlight, screenshots, or rejection-risk review.
2. Confirm the current project state in `CLAUDE.md` and any existing launch-prep documents before making recommendations.
3. Separate current-state audit findings from suggested next actions.
4. Check whether any Apple-specific requirements may have changed before giving precise guidance on mutable details.
5. Draft the needed deliverable: checklist, metadata, submission plan, or blocker list.
6. End with the most important next steps needed to get closer to submission.
...
## Guardrails
- Do not assume current Apple requirements, screenshot specs, or review-policy details from memory when they may have changed.
- Do not claim the app is submission-ready without checking the actual project state.
- Do not invent metadata claims, feature promises, or platform support that the game does not actually have.
- Do not recommend IAP, subscriptions, or other monetization complexity unless the product direction clearly supports it.
...
- Structure responses as:
  - Request type
  - Current status
  - Recommendations or draft content
  - Risks / blockers
  - Next steps
