// Revelia/ViewModels/GameViewModel.swift
...
class GameViewModel: ObservableObject {
    ...
    init(levelSpec: LevelSpec, suspendedRun: SuspendedRun) {
        self.levelSpec = levelSpec
        self.seed = suspendedRun.seed
        self.board = suspendedRun.board
        self.gameState = suspendedRun.gameState
        self.stats = suspendedRun.stats
        self.score = suspendedRun.score
        self.stars = suspendedRun.stars
        self.elapsedTime = suspendedRun.elapsedTime
        self.beaconChargesRemaining = suspendedRun.beaconChargesRemaining
        self.isBeaconTargeting = suspendedRun.isBeaconTargeting
        self.conductorChargesRemaining = suspendedRun.conductorChargesRemaining
        self.isConductorTargeting = suspendedRun.isConductorTargeting
        self.illuminatedCoords = suspendedRun.illuminatedCoords
        self.linkedHighlightedCoord = suspendedRun.linkedHighlightedCoord
        self.lockedSonarCoords = suspendedRun.lockedSonarCoords
        self.explosionOrigin = suspendedRun.explosionOrigin
        self.quicksandFadeProgress = suspendedRun.quicksandFadeProgress

        if suspendedRun.gameState == .playing {
            startTimer()
    ...
    func makeSuspendedRun() -> SuspendedRun? {
        guard [.waitingForFirstScan, .playing, .paused].contains(gameState) else { return nil }

        return SuspendedRun(
            schemaVersion: SuspendedRun.currentSchemaVersion,
            ...
            beaconChargesRemaining: beaconChargesRemaining,
            isBeaconTargeting: isBeaconTargeting,
            conductorChargesRemaining: conductorChargesRemaining,
            isConductorTargeting: isConductorTargeting,
            illuminatedCoords: illuminatedCoords,
            linkedHighlightedCoord: linkedHighlightedCoord,
            lockedSonarCoords: lockedSonarCoords,
            explosionOrigin: explosionOrigin,
            quicksandFadeProgress: quicksandFadeProgress,
            savedAt: Date()
        )
    }
    ...
    private func handleWin() {
        ...
        score = ScoringCalculator.calculateScore(stats: stats, level: levelSpec)
        stars = ScoringCalculator.calculateStars(stats: stats, level: levelSpec)
    }
