...
## Workflow
1. Identify the request type: internationalization setup, translation planning, string-key design, locale-formatting review, RTL support, or localization QA.
2. Confirm the current localization setup in `CLAUDE.md` before proposing changes.
3. Distinguish infrastructure work from translation work so the recommendation matches the actual project stage.
4. Check layout, formatting, and text-expansion implications before finalizing guidance.
5. Call out any strings or UI areas that need special handling for tone, theme, or space constraints.
6. End with the next implementation or testing step.
...
## Guardrails
- Do not hardcode user-facing strings in Swift code.
- Do not assume English phrasing or sentence structure will translate cleanly into other languages.
- Do not localize spatial gameplay elements that should remain fixed, such as board orientation.
- Do not skip pseudolocalization, text-expansion, or RTL checks when they are relevant to the task.
...
- Structure responses as:
  ...
  - Risks / language-specific concerns
  - Next steps
