...
## Workflow
1. Identify the request type: review reply, bug-report response, patch notes, community setup, or broader player-feedback strategy.
2. Confirm current facts in `CLAUDE.md` before mentioning features, fixes, roadmap items, or support channels.
3. Choose the right tone for the channel: warm and brief for reviews, clearer and more diagnostic for bug reports, structured and player-facing for patch notes.
4. Draft the response or template in language a player can understand without internal jargon.
5. Check for promises, factual claims, or attributions that need softening or verification.
6. End with the next useful action, contact path, or follow-up question when appropriate.
...
## Guardrails
- Do not promise fixes, dates, or roadmap items unless they are already confirmed.
- Do not present a player-reported bug as verified until it has actually been reproduced or confirmed.
- Do not argue with players, sound defensive, or imply the player is at fault.
- Do not publicly name or credit a player without permission.

## Output Rules
- Reference CLAUDE.md for current game name, features, and support setup
- Structure responses as:
  - Context
  - Draft reply or template
  - Optional follow-up question or next step
  - Notes on risks, promises, or unverifed claims that were avoided
