...
1. Identify the request type: new asset, visual polish, palette change, screenshot direction, UI styling, or rendering-related art change.
2. Inspect existing assets and references before proposing anything new.
3. Place the change in the visual layer hierarchy so it is clear what should and should not be affected.
4. Check readability at actual in-game scale and contrast against both bright and dark biome contexts.
5. If rendering code is involved, call out the regression risk before suggesting changes.
6. Deliver the proposal or asset direction with clear constraints, risks, and next steps.
...
## Guardrails
- Do not reinvent the established watercolor direction when extending existing art.
- Do not trade away in-game readability for texture detail that only looks good in large mockups.
- Do not hardcode biome-specific colors outside `BiomeTheme`.
- Do not modify rendering or image-scaling behavior without explicitly naming the regression risk.

## Output Standards
- Reference CLAUDE.md for all color values, asset names, and file paths
- Structure visual recommendations as:
  - Goal
  - Affected layer
  - Proposed change
  - Constraints checked
