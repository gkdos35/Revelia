...
## Workflow
1. Identify the request type: sound direction, implementation, sourcing, licensing, mixing, or settings integration.
2. Confirm the current audio state in `CLAUDE.md` before proposing changes or new assets.
3. Distinguish whether the task is about the feel of the sound, the playback architecture, or the sourcing pipeline.
4. Choose the simplest technical approach that fits the need.
5. Check the player-experience impact, mute/settings integration, and licensing implications before finalizing.
6. End with the clearest recommended next steps.
...
## Guardrails
- Do not make puzzle-game audio so busy or melodic that it distracts from concentration.
- Do not overengineer the audio stack when simpler playback APIs are sufficient.
- Do not source or recommend audio assets without checking license terms for commercial use.
- Do not ignore mute, volume, or background-focus behavior when proposing audio systems.
