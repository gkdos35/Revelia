// Revelia/Views/BiomeSelectView.swift
//
...
struct BiomeSelectView: View {
    ...
    /// Called when the player opens High Scores from the title screen overlay.
    var onShowHighScores: () -> Void = {}
    ...
    private var continentMapAssetName: String {
        showingHex ? "ContinentMapHex" : "ContinentMap"
    }

    private var campaignTogglePreviewAssetName: String {
        showingHex ? "ContinentMap" : "ContinentMapHex"
    }
    ...
    private var mapContent: some View {
        GeometryReader { geo in
            ...
            .onChange(of: showTitleScreen) { _, showing in
                DispatchQueue.main.async {
                    audioManager.transition(to: showing ? .home : .biomeMap)
                }
            }
        }
    ...
    private var chromeOverlay: some View {
        VStack(spacing: 0) {
            ...
            // Bottom controls: specimen cabinet on the left, globe toggle on the right.
            HStack(alignment: .bottom, spacing: 0) {
                specimenCabinetButton
                Spacer()
    ...
    private var specimenCabinetButton: some View {
        VStack(spacing: 2) {
            Button {
                audioManager.playMenuClick()
                showingCabinet = true
            } label: {
                specimenCabinetIllustration
            }
            .buttonStyle(.plain)
            .padding(.bottom, -10)

            VStack(spacing: 4) {
                Text("\(specimenStore.unlockedSpecimenIds.count) of \(SpecimenCatalog.all.count)")
                    .font(.system(size: 11, weight: .semibold, design: .rounded))
                    .foregroundStyle(Color(red: 0.97, green: 0.90, blue: 0.72))
            }
            .frame(width: 90)
            .multilineTextAlignment(.center)
        }
        .help("Open the Specimen Cabinet")
    }

    private var specimenCabinetIllustration: some View {
        Image("DisplayShelf")
            .resizable()
            .scaledToFit()
    ...
    private var campaignGlobeButton: some View {
        Button {
            ...
            ZStack {
                ...
                // Preview the destination campaign, not the active one.
                Image(campaignTogglePreviewAssetName)
                    .resizable()
                    .scaledToFill()
...
struct BiomeSelectView_Previews: PreviewProvider {
    static var previews: some View {
        BiomeSelectView(onSelect: { _ in })
            .previewDisplayName("Map — fresh start")
            .environmentObject(ProgressStore())
            .environmentObject(SettingsStore())
