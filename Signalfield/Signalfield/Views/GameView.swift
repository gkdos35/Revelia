...
struct GameView: View {
    ...
    /// The pre-mounted SKView (always in the ZStack) detects a non-nil scene
    /// in updateNSView and calls presentScene — didMove(to:) fires on the next
    /// frame (~16 ms), starting the animation with near-zero delay.
    ...
    var body: some View {
        ...
        ZStack {
          ...
          VStack(spacing: 16) {
            ...
            ZStack {
                ...
                // SpriteKit explosion overlay — ALWAYS in the tree so the SKView
                // is pre-mounted in the window hierarchy. Creating an NSView at
                // explosion time costs ~1 s on macOS; pre-mounting avoids this.
                // The SKView starts empty. When explosionScene goes from nil to
                // non-nil, updateNSView presents the scene and didMove(to:) fires
                // on the next frame (~16 ms). The transparent background means it's
                // invisible until tile sprites are added by runExplosion().
        ...
        .onChange(of: viewModel.gameState) {
            ...
            // on the pre-mounted SKView. didMove(to:) fires on the next frame
