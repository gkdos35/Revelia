// SignalfieldTests/SpecimenCatalogTests.swift
//
...
final class SpecimenCatalogTests: XCTestCase {
    ...
    func testSpecimenForL1IsTrainingRange1() {
        let s = SpecimenCatalog.specimen(for: "L1")
        XCTAssertNotNil(s, "L1 should have a specimen")
        XCTAssertEqual(s?.name, "Training Range Specimen 1")
    ...
    func testSpecimenForL6IsTrainingRange6() {
        let s = SpecimenCatalog.specimen(for: "L6")
        XCTAssertNotNil(s)
        XCTAssertEqual(s?.name, "Training Range Specimen 6")
        XCTAssertEqual(s?.biomeId, 0)
        XCTAssertFalse(s?.isHex ?? true)
    }

    func testSpecimenForL75IsTrainingRangeHex1() {
        // L75 is the first hex Training Range level → "Training Range Specimen 7"
        // (square specimens 1–6 come first, hex starts at 7)
        let s = SpecimenCatalog.specimen(for: "L75")
        XCTAssertNotNil(s, "L75 should have a specimen")
        XCTAssertEqual(s?.name, "Training Range Specimen 7")
        XCTAssertEqual(s?.biomeId, 0)
        XCTAssertTrue(s?.isHex ?? false, "L75 is a hex level")
        XCTAssertFalse(s?.isRare ?? true)
    }

    func testSpecimenForL80IsTrainingRangeHex6() {
        // L80 is the last hex Training Range level → "Training Range Specimen 12"
        let s = SpecimenCatalog.specimen(for: "L80")
        XCTAssertNotNil(s)
        XCTAssertEqual(s?.name, "Training Range Specimen 12")
        XCTAssertEqual(s?.biomeId, 0)
        XCTAssertTrue(s?.isHex ?? false)
    }

    func testSpecimenForL148IsHexDelta12() {
        // L148 is the last level in the hex Delta → biome 8, last hex level specimen
        // Biome 8 has 12 sq specimens (1–12) and 12 hex specimens (13–24)
        let s = SpecimenCatalog.specimen(for: "L148")
        XCTAssertNotNil(s)
        XCTAssertEqual(s?.biomeId, 8)
        XCTAssertTrue(s?.isHex ?? false)
        XCTAssertEqual(s?.name, "The Delta Specimen 24")
        XCTAssertFalse(s?.isRare ?? true)
    }
    ...
    func testSquareRareSpecimenBiome0() {
        ...
        XCTAssertEqual(s?.name, "Training Range Rare Specimen")
        XCTAssertEqual(s?.biomeId, 0)
    ...
    func testHexRareSpecimenBiome0() {
        ...
        XCTAssertEqual(s?.name, "Training Range Hex Rare Specimen")
        XCTAssertEqual(s?.biomeId, 0)
        XCTAssertTrue(s?.isRare ?? false)
        XCTAssertTrue(s?.isHex ?? false)
    ...
    func testAllSpecimensUseplaceholderImageName() {
        let wrongImages = SpecimenCatalog.all.filter { $0.imageName != "specimen-placeholder" }
        XCTAssertTrue(wrongImages.isEmpty,
            "All specimens should use 'specimen-placeholder' until real art is ready. " +
            "Found \(wrongImages.count) with different imageName: \(wrongImages.map { $0.id })")
    }
